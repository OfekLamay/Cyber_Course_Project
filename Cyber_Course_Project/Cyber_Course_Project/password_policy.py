...
class PasswordPolicy:
    ...
    # NEW: password history length (deny reuse of last N)
...
def _policy_file_path() -> Path:
    ...
    d:\...\Cyber_Course_Project\Cyber_Course_Project\Cyber_Course_Project\config\password_config.json

    If settings.BASE_DIR is d:\...\Cyber_Course_Project\Cyber_Course_Project,
    then BASE_DIR / "Cyber_Course_Project/config/password_config.json" matches.
    """
...
def load_password_policy() -> PasswordPolicy:
    """
    Loads and validates policy from password_config.json.
    No hardcoded fallback policy: if config is wrong/missing, raises ImproperlyConfigured.
    """
    ...
    # Optional key with safe default
    phc = pp.get("password_history_count", 3)
    if not isinstance(phc, int):
        raise ImproperlyConfigured(f"{cfg_path}: password_policy.password_history_count must be int")
    if phc < 0:
        raise ImproperlyConfigured(f"{cfg_path}: password_policy.password_history_count must be >= 0")
    ...
    # Basic sanity checks (still config-driven; just rejects invalid configurations)
